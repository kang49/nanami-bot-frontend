...
const computeYAxisBounds = (dataPoints) => {
    ...
    if (allYValues.length === 0) return { min: 0, max: 100 }; // Update max if expected more than 100 MB
...
const initialChartOptions = {
    ...
    fill: {
        ...
        gradient: {
            shade: 'dark',
            gradientToColors: ['#34a853'], // Changed the color for differentiation
            shadeIntensity: 1,
            type: 'horizontal',
            opacityFrom: 1,
            opacityTo: 1,
    ...
    yaxis: {
        labels: {
            formatter: function (val) {
                return val.toFixed(2) + ' MB'; // This will round the values to two decimal places and add 'MB' unit
            }
        },
        min: 0, // Adjust according to your minimum expected value
        max: 1000 // Adjust according to your maximum expected value
    },
    responsive: [
        {
            breakpoint: 767,
...
watch(() => props.botstatusData, (newData) => {
    if (newData && newData.memory_usage !== undefined) {
        const now = new Date();
        series.value[0].data.push({ x: now, y: (newData.memory_usage).toFixed(1) });
